<div class="container block">
    <section class="text-xs-center">
        <p>This is a personal project</p>
        <p>This server has 1 CPU and 1 GB RAM, results in seconds are higher than a regular server</p>
        <p>Time in seconds is an average for all tests (one daily) for each test/version</p>
        <p>Using <a href="https://github.com/eusonlito/phpfarm" target="_blank"><strong>https://github.com/eusonlito/phpfarm</strong></a> to perform tests</p>
        <p>More info and collaborations in <a href="https://github.com/eusonlito/php-microoptimizations" target="_blank"><strong>GitHub</strong></a></p>
    </section>
</div>
